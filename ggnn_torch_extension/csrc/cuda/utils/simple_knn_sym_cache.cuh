...
/**
 * @brief A specialized version of the KNN cache for the symmetric link search.
 *
 * This cache is unique because it needs to calculate two distances simultaneously:
 * 1. The distance from the target point ('n' in the sym_query_kernel).
 * 2. The distance from a midpoint between 'n' and its neighbor, to ensure good
 *    graph connectivity for queries that might fall between two points.
 */
...
struct SimpleKNNSymCache {
 ...
 private:
  ...
  using BlockReduceDistAndNorm = cub::BlockReduce<DistQueryAndHalf, BLOCK_DIM_X>;

  struct CacheTempStorage {
    typename BlockReduceDist::TempStorage dist_reduce;
    typename BlockReduceDistAndNorm::TempStorage dist_query_half_reduce;
  };
 ...
 public:
  ...
  __device__ __forceinline__ SimpleKNNSymCache(uint32_t D_in, DistanceMeasure measure_in,
                                               uint32_t BEST_SIZE_in, uint32_t SORTED_SIZE_in,
                                               uint32_t CACHE_SIZE_in, const BaseT* d_base_in,
                                               const KeyT n, ValueT xi_criteria)
      : D(D_in),
        measure(measure_in),
        BEST_SIZE(BEST_SIZE_in),
        SORTED_SIZE(SORTED_SIZE_in),
  ...
  __device__ __forceinline__ void loadQueryPos(const BaseT* d_query)
  {
    ...
    if (measure == DistanceMeasure::Cosine) {
      ...
      r0_query_norm =
          typename SimpleKNNSymCache::BlockReduceDist(s_storage.dist_reduce).Sum(query_norm);
      __syncthreads();
    }
  }
  ...
  __device__ __forceinline__ void init_start_point(const KeyT other_n, const KeyT* d_translation)
  {
    ...
    __syncthreads();
    if (measure == DistanceMeasure::Cosine) {
      DistQueryAndHalf norms_sum =
          typename SimpleKNNSymCache::BlockReduceDistAndNorm(s_storage.dist_query_half_reduce)
  ...
  __device__ __forceinline__ DistQueryAndHalf distance_synced(const KeyT other_id)
  {
    ...
    dist = typename SimpleKNNSymCache::BlockReduceDistAndNorm(s_storage.dist_query_half_reduce)
               .Reduce(dist, DistQueryAndHalf::Sum());
    if (measure == DistanceMeasure::Cosine) {
      __syncthreads();
      // need to normalize by the vectors' lengths (in high dimensions, no
      // vector has length 1.0f)
      norm_other = typename SimpleKNNSymCache::BlockReduceDist(s_storage.dist_reduce).Sum(norm_other);
      if (!threadIdx.x) {
