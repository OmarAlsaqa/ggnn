...
{
  const float growth =
      std::pow(static_cast<float>(params.N) / static_cast<float>(S), 1.f / (L - 1));
...
GraphConfig::GraphConfig(uint32_t n_in, uint32_t d_in, uint32_t k_build_in)
    // First, initialize the base GraphParameters struct
    : GraphDerivedParameters{GraphParameters{.N = n_in, .D = d_in, .KBuild = k_build_in}},
      // Then, the GraphDimensions struct can be initialized from the now-complete
      // GraphDerivedParameters
      GraphDimensions{static_cast<const GraphDerivedParameters&>(*this)}
{
}
                              ...
                              int refinement_iterations, ggnn::DistanceMeasure measure)
{
  ...
  TORCH_CHECK(base_type == torch::kFloat32 || base_type == torch::kUInt8,
              "base_tensor must be of type float32 or uint8.");

  // 2. --- Configuration ---
  ggnn::GraphParameters params{.N = (uint32_t)base_tensor.size(0),
                               .D = (uint32_t)base_tensor.size(1),
                               .KBuild = (uint32_t)k_build};
  ggnn::GraphConfig config(params.N, params.D, params.KBuild);
  ...
  // Allocate the temporary workspace buffer. It will be freed automatically when it goes out of
  // scope.
  ...
  const auto& type_info =
      base_tensor.dtype().toScalarType() == torch::kFloat ? typeid(float) : typeid(uint8_t);

  cudaStream_t stream = at::cuda::getCurrentCUDAStream();

  // 5. --- Launch the CUDA Backend ---
  ggnn::cuda::launch_graph_construction(config, graph_view, buffer_view, base_ptr, type_info,
                                        tau_build, measure, (uint32_t)refinement_iterations,
                                        stream);
...
std::tuple<torch::Tensor, torch::Tensor> query_graph_cpp(
    torch::Tensor query_tensor, torch::Tensor graph_tensor, torch::Tensor base_tensor, int k_build,
    int k_query, float tau_query, int max_iterations, ggnn::DistanceMeasure measure)
{
  // 1. --- Input Validation ---
  TORCH_CHECK(query_tensor.is_cuda() && graph_tensor.is_cuda() && base_tensor.is_cuda(),
              "All input tensors must be CUDA tensors.");
  TORCH_CHECK(query_tensor.dim() == 2, "Query tensor must be 2D.");
  TORCH_CHECK(query_tensor.device() == base_tensor.device() &&
                  query_tensor.device() == graph_tensor.device(),
              "All tensors must be on the same CUDA device.");
  TORCH_CHECK(query_tensor.dtype() == base_tensor.dtype(),
              "Query and base tensors must have the same dtype.");
  ...
  const auto& type_info =
      query_tensor.dtype().toScalarType() == torch::kFloat ? typeid(float) : typeid(uint8_t);
                                                      ...
                                                      ggnn::DistanceMeasure measure)
{
  ...
  TORCH_CHECK(base_tensor.dtype() == query_tensor.dtype(),
              "Base and query tensors must have the same dtype.");
  TORCH_CHECK(base_tensor.dim() == 2 && query_tensor.dim() == 2, "Both tensors must be 2D.");
  TORCH_CHECK(base_tensor.size(1) == query_tensor.size(1),
              "Base and query tensors must have the same number of dimensions (D).");
  TORCH_CHECK(k > 0, "k must be greater than 0.");
  TORCH_CHECK(base_tensor.is_contiguous() && query_tensor.is_contiguous(),
  ...
  ggnn::cuda::launch_bf_query(base_tensor.data_ptr(), query_tensor.data_ptr(), type_info,
                              result_ids.data_ptr<ggnn::cuda::KeyT>(),
                              result_dists.data_ptr<ggnn::cuda::ValueT>(), base_tensor.size(0),
                              query_tensor.size(0), query_tensor.size(1), k, measure, stream);
